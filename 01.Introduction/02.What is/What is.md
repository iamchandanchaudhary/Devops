...
- Requirement Gathering and Analysis: Information is collected about product features, user requirements, market state, and other factors.
- Planning: Determines what is needed, including cost, resources, and risks associated with the product implementation.
- Designing: Architects design the software based on previous inputs and produce design documents serving as roadmaps for developers.
- Development: Developers write the software code based on the design.
- Testing: Software testers test the software for defects. The software is promoted to production only after fixing all issues.
- Deployment: Software is deployed to the production environment for users.
- Maintenance: Balances regular changes and system uptime.

Let's review the entire process again:

1. Requirement gathering
2. Planning
3. Designing by architects
4. Development by developers
5. Testing by software testers
6. Deployment and maintenance by operations team
...
These models represent different paths to reach the same destination, chosen based on factors like cost, risk, and time.

In the Waterfall model, each phase must be completed before the next begins. For example, requirements must be complete before planning, and development starts after planning. Testing and maintenance follow after all features are developed. However, it is difficult to go back and change something once a phase is completed, and working software is produced very late in the lifecycle, which may take months.

Emma is unsure about all the requirements at once. She would like to observe product development and alter or add new ideas along the way. Therefore, the Waterfall model is not suitable.
...
Avi is unhappy with the operations team and tells Freddy that these issues cannot continue. Development is Agile, focusing on regular and quick changes, while operations is ITIL-driven, providing a stable environment. There is a big wall of confusion between these two parties. Developers throw their code over the wall, and operations are responsible for deployment. Developers complain about deployment delays, and operations complain about unclear instructions and workload.
...
He trains the development team on operations concepts to improve communication with operations. He also trains the operations team on Agile concepts to enhance collaboration with development. Most importantly, he provides automation training and instructions to every team across the board.

Automation is applied to every task in the code delivery process, including code build, code testing, software testing, infrastructure changes, deployments, and everything along the way. The wall of confusion is brought down like the Berlin Wall, and the teams decide to work together collaboratively.
...
- Agile SDLC allows iterative development with frequent feedback, but can stress operations teams with frequent deployments.
- DevOps bridges the gap between development and operations through collaboration, communication, and automation, resulting in faster and more reliable software delivery.